...
int main()
{
	// glfw initialize and configure（GLFW的作用是创建OpenGL context和窗口的抽象）
	// -----------------------------
	glfwInit();
	...
	// --------------------------------
	Shader lightingShader("01colors/1.colors.vs", "01colors/1.colors.fs");
	Shader lightCubeShader("01colors/1.light_cube.vs", "01colors/1.light_cube.fs");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// render the cube
		// 已经对lighting shader进行了设置，进行被光源照射着的物体的渲染
		glBindVertexArray(cubeVAO);
		glDrawArrays(GL_TRIANGLES, 0, 36);

		// render the light object
		// 然后设置light cube shader，渲染light cube
		lightCubeShader.use();
		lightCubeShader.setMat4("projection", projection);
		lightCubeShader.setMat4("view", view);
		// light cube跟cube共用一组存在VBO中的顶点，将light cube尺寸缩小并平移到lightPos
		model = glm::mat4(1.0f);
