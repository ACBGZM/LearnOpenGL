...
int main()
{
	...
	Shader shader("04textures/shader.vs", "04textures/shader1.fs");
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);	// 纹理缩小（一个像素对应一块纹理）时，使用层间线性插值的Mipmap，每层内部也线性插值
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// bind texture
		glBindTexture(GL_TEXTURE_2D, texture);	// 绑定纹理，自动把纹理赋值给fragment shader的uniform sampler采样器
