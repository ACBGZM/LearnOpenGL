...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// 定义随时间变化的变换矩阵
		// 需要在每一次迭代中创建变换矩阵，从而保证能不断更新角度等变换参数
		glm::mat4 trans;	// glm的mat4跟GLSL的mat4可以共通
		// 顺序：先scale、再围绕原点rotate、再translate到右下角
		trans = glm::rotate(trans, (float)glfwGetTime(), glm::vec3(0.0f, 0.0f, 1.0f));
		trans = glm::translate(trans, glm::vec3(0.5f, -0.5f, 0.0f));	// 平移
		trans = glm::scale(trans, glm::vec3(0.5f, 0.5f, 0.5f));	// 三个轴缩小成0.5倍

		// 把trans传递到GLSL的uniform变量
		...
		glm::mat4 trans2;
		// 顺序：先scale、再围绕原点rotate、再translate到右下角
		trans2 = glm::translate(trans2, glm::vec3(0.5f, -0.5f, 0.0f));	// 平移
		trans2 = glm::rotate(trans2, glm::radians(180.0f), glm::vec3(0.0f, 0.0f, 1.0f));
		float t = std::abs(std::sin(glfwGetTime() * 2));
		trans2 = glm::scale(trans2, glm::vec3(t, t, t));	// 三个轴随时间缩放
