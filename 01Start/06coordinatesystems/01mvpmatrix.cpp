// LOCAL SPACE									物体起始的坐标是对象相对于局部原点的坐标
// ->(model matrix)-> WORLD SPACE				更大的空间范围内，所有物体都相对于世界原点摆放
// ->(view matirx)-> VIEW SPACE					从摄像机的角度进行观察
// ->(projection matrix)-> CLIP SPACE			坐标被处理到[-1, 1]，并将超过投影矩阵指定的frustum范围的坐标忽略
// ->(viewport transformation)-> SCREEN SPACE	坐标进入由glViewport定义的坐标范围内，送入光栅器，转化成fragment

// 将顶点变换到不同坐标空间的原因：有些操作在特定坐标系统中才有意义且更方便。
// 如：在局部坐标对物体进行修改；在世界坐标中进行物体相对于另一个物体的位置变化
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glm::mat4 view;
		glm::mat4 projection;
		model = glm::rotate(model, glm::radians(-55.0f), glm::vec3(1.0f, 0.0f, 0.0f));	// 绕x轴旋转，看上去像放在地上
		view = glm::translate(view, glm::vec3(0.0f, 0.0f, -0.5f * glfwGetTime()));	// 实现摄像机向后移动的效果，也就是物体像前移动，也就是向-z方向移动
