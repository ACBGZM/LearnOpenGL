...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glm::mat4 view;
		glm::mat4 projection;
		model = glm::rotate(model, (float)glfwGetTime() * glm::radians(50.0f), glm::vec3(0.5f, 1.0f, 0.0f));	// 绕x轴旋转，看上去像放在地上
		view = glm::translate(view, glm::vec3(0.0f, 0.0f, -0.1f * glfwGetTime()));	// 实现摄像机向后移动的效果，也就是物体像前移动，也就是向-z方向移动
		...
		// render container
		glBindVertexArray(VAO);
		glDrawArrays(GL_TRIANGLES, 0, 36);	// 直接画6个面、12个三角形、36个顶点（没用索引EBO）
