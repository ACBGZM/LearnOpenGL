...
"	vertexColor = vec4(0.0f, 0.5f, 0.1f, 1.0f);\n"	// 设置输出变量
"}\0";
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// render
		// ------
		glClearColor(0.5f, 0.3f, 0.2f, 0.3f);
		glClear(GL_COLOR_BUFFER_BIT);

		// be sure to activate the shader before any calls to glUniform
		glUseProgram(shaderProgram);

		// 修改在fragment shader 定义的 uniform vec4 ourColor
		float timeValue = glfwGetTime();
		float greenValue = (sin(timeValue) / 2.0f) + 0.5f;
		int uniColorLocation = glGetUniformLocation(shaderProgram, "ourColor");	// 查找uniform全局变量的位置值，没有位置会返回-1
		glUniform4f(uniColorLocation, 0.0f, greenValue, 0.0f, 1.0f);	// 必须先使用shaderProgram再在当前激活的着色器程序中设置uniform
